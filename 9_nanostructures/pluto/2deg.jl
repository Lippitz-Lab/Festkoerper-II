...
begin
	...
	field[ 0 .<= x  .< dn ] .= 1 ./ dn
	field = cumsum(field)
	field = field ./ abs(minimum(field))
end;

# ╔═╡ 4228bad7-2de3-4cd4-a54b-4e5a5fa0f2f2
begin
	phi = zeros(size(x));
	phi[ -dp .<= x  .< 0  ] .= n_A .* (dp .+ x[ -dp .<= x  .< 0  ]).^2
	phi[ 0 .<= x  .< dn ] .= 1 .- n_D  .* (dn .- x[ 0 .<= x  .< dn  ]).^2
	phi[ x .>= dn] .= 1
end;

# ╔═╡ f6f1c097-325b-48c2-92bc-e4f4224b1d0e
begin
	CB = -phi .+ 1.1
	VB = -phi .- 0.9;
	CB2 = copy(CB)
...
begin
		...
		myaxis = @pgf PGFPlotsX.Axis(
	    {
		  ...
		  ymax = 1.8,
	       xmin =-1.1, xmax =1.1,
			...
			xlabel=raw"Wachstumsrichtung $z$",
			ytick = [], 
		#	xtick = (0:5:25),
			xtick = [ 0,],
			xticklabels = {""},
	...
	push!(myaxis, raw"\node[below right] at (0,0) {\footnotesize 2DEG};")
	 ...
	 push!(myaxis, raw"\draw[thin] (-1,0) -- (1,0) node [below] {$E_F$};")

	
	
	
	pgfsave("../2DEG.tikz.tex",myaxis; include_preamble= false)
	myaxis
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "46bd215ec3a431e10c8ce9b089880d4a833d08a9"
