...
\begin{equation}
   \ell = \frac{1}{n_{ph} \, \sigma_{st}} = \tau \, v_F \quad ,
\end{equation}
wobei wir hier den Streuquerschnitt $\sigma_{st}$ genannt haben, um ihn von der Leitfähigkeit $\sigma$ zu unterscheiden, und $n_{ph}$ die Dichte der Phononen bezeichnet. $\sigma_{st}$ ändert sich bei diesen Temperaturen kaum, ist also für unsere Zwecke konstant. Elektronen bewegen sich mit der Fermi-Geschwindigkeit $v_F$, die so den Zusammenhang zwischen Stoßzeit und Weglänge herstellt.

Bei einer Temperatur $T$ (viel) größer als der Debye-Temperatur $\Theta$ ändert sich die Dichte $n_{ph}$ der Phononen wie $T/\Theta$, so dass wir für die Leitfähigkeit $\sigma$ erhalten
\begin{equation}
   \sigma \propto \left\{ 
      \begin{matrix}
   ...
   \right.  \quad .
\end{equation}
Bei hohen Temperaturen ist die Leitfähigkeit also völlig durch die Dichte $n_{ph}$ der Phononen bestimmt.
