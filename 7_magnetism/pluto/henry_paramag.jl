...
# ╔═╡ 2f6588fe-b1e5-4745-9536-6206af2394ce
d = CSV.read("../data/henry_paramag.csv", DataFrame);

# ╔═╡ abee7d1f-8bb2-46c5-a794-37459041aee9
B(x,J) = (2*J +1)/(2*J) * coth( (2*J +1)/(2*J) * x) - 1/(2*J)* coth( 1/(2*J) * x);

# ╔═╡ 787a7135-5980-4382-b5d4-f0ceddce6714
begin
	x = (0:0.01:4)
	scatter(d.x ./ 10, d.Curve1)
	scatter!(d.x ./10 , d.Curve2)
	scatter!(d.x ./ 10, d.Curve3)
	plot!(x, B.(x * 1.3 * 3/2 , 3/2) .* 3)
	plot!(x, B.(x * 1.3 * 5/2 , 5/2) .* 5)
	plot!(x, B.(x * 1.3 * 7/2 , 7/2) .* 7)
	plot!(legend=false, xlabel ="B/T (T/K)", ylabel = "mu (mu_b/mol)")
end

# ╔═╡ fc0cbf3f-e5b8-45cb-8445-a2f4b1c9ea95
out = DataFrame("x" => x, "M1" => B.(x * 1.3 * 3/2 ,3/2) .* 3, "M2" => B.(x * 1.3 * 5/2 ,5/2) .* 5, "M3" => B.(x * 1.3 * 7/2 ,7/2) .* 7);

# ╔═╡ f3e679e6-d1a2-4647-9c6d-dc389de1bdec
CSV.write("./henry_paramag_model.csv", out;)
...
manifest_format = "2.0"
project_hash = "52d7528f781811ecd341149b9194517601a0d2db"
