...
Der von einem supraleitenden Zylinder oder einer supraleitenden geschlossenen Leiterschleife umschlossene magnetische Fluss\sidenote{Feldstärke mal Fläche} ist quantisiert. Dies haben 1961 gleichzeitig R. Doll \& M. Näbauer in München und B.S. Deaver \& W.M. Fairbank in Stanford experimentell gefunden.
...
\end{equation}
mit dem Vektorpotential\sidenote{ mit $\nabla \times \bm{A} = \bm{B}$ und der London-Eichung $\nabla \cdot \bm{A} = 0$ und $\bm{A}_\perp = 0$} $\bm{A}$ und  der London-Länge 
\begin{equation}
...
\begin{marginfigure}
    ...
    \caption{Strom durch einen \ch{Pb}/\ch{PbO_x}/\ch{Pb} Tunnelkontakt (Daten aus \cite{Langenberg1966}). Der Sprung in der Zustandsdichte ist bei $eU=2\Delta$.
