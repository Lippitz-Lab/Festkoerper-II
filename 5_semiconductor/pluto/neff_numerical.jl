...
fn(n,T) = ( n * (n_A + n) / (n_D - n_A - n) ) - Neff * exp(-E_d / T);

# ╔═╡ d32fa1d3-f9b0-48f8-8b4e-27fc695347c5
n(T) = find_zero(x -> fn(x,T), (0 , n_D - n_A )) + Neff * exp(-E_c / (2*T));

# ╔═╡ a525f13e-fbc1-4046-af1e-ca0b3990a68a
EF(T) = log(n(T) / Neff) * T + E_c;

# ╔═╡ 6b7d6be6-cf4b-42e5-b5f5-b6aea55eb960
let
tr = (0:0.1:25);
	ted = (14:0.1:25);
	ted2 = (5:0.1:20);
	...
	plot(tr, @. EF(1 ./ tr) ; ylim=(5,10))
	
	plot!(ted, @. E_c - E_d + (1 / ted) * log(n_D/n_A) )
	plot!(ted2, @. E_c - E_d/2 - (1 / (2*ted2)) * log(Neff/n_D) )
	plot!(tc, @. E_c - (1 / tc) * log(Neff/n_D) )
...
let
	...
	plot(tr, @. n(1 ./ tr) ; yaxis=:log)
	plot!(ted, @. 1.5 * 10^5 * exp(- E_d * ted) ; yaxis=:log)
	plot!(ted2, @. 4 * 10^1 * exp(- E_d * ted2 / 2) ; yaxis=:log)
	plot!(tc, @. tc .* 0 + 1.5  ; yaxis=:log)
	plot!(tg, @. 11 * 10^1 * exp(- E_c * tg / 2) ; yaxis=:log)

end

# ╔═╡ bf226710-9fa4-4ed9-8648-cd95488a911d
begin
	tr = (0:0.1:25);
	ted = (14:0.1:25);
	ted2 = (5:0.1:20);
...
@pgf gp = GroupPlot(
	...
	@pgf    p = PGFPlotsX.Plot(
	        { no_marks, dashed },
	        Table([ted2, @. 4 * 10^1 * exp(- E_d * ted2 / 2) ] ) )
	push!(myaxis, p)
	...
	@pgf    p = PGFPlotsX.Plot(
	        ...
	        Table([tr,  @. EF(1 / tr) / 10 ] ) )
		push!(myaxis2, p)
		push!(myaxis2, raw"\draw[<->] (1,0.5) -- node {$E_g/2$} (1,1);")
		push!(myaxis2, raw"\draw[<->] (2,0.9) -- node[right] {$E_d$} (2,1);")
	...
	pgfsave("../doping_temp.tikz.tex",gp; include_preamble= false)
	gp
end
...
manifest_format = "2.0"
project_hash = "6ed4767851798b666b4e97f78923d24b849cd10d"
